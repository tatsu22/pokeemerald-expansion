...
void ItemUseCB_RareCandy(u8 taskId, TaskFunc task)
{
    ...
    if (sInitialLevel != MAX_LEVEL && !IsLevelCapped(mon)) // MY CHANGES cannot use rare candy on level capped pokemon
    {
...
// Mega rare candy code is the same as the rare candy, but it doesn't remove the item from the bag
void ItemUseCB_MegaRareCandy(u8 taskId, TaskFunc task)
{
    struct Pokemon *mon = &gPlayerParty[gPartyMenu.slotId];
    struct PartyMenuInternal *ptr = sPartyMenuInternal;
    s16 *arrayPtr = ptr->data;
